...
class ViewScale {
  ...
  init() {
    scaleUnits.unitNames.forEach((unitName) => {
      const barUnit = createElementSlider([
        'js-range-slider__bar-unit',
        'range-slider__bar-unit',
        `${unitName}`,
      ]);
      const unitValue = createElementSlider([
  ...
  update(coordinates: string[], maxValue: number, minValue: number): void {
    const size = coordinates[0] === 'vertical'
    ...
    this.units.forEach((unit, index) => {
      ...
      if (index === 0) {
        ...
        unit.setAttribute(
          'style',
          `${coordinates[1]} ${scaleUnits.unitPositions[index]}%`,
        );
        children[0].textContent = (
          minValue
          + 0.01 * scaleUnits.unitPositions[index] * (maxValue - minValue)
        ).toString();
      }
    });
